  ...
  return (
    <div className="min-h-screen bg-[#edf1f5] text-[#222222] flex flex-col justify-between font-sans">
      <div>
        ...
        <main className="max-w-4xl mx-auto bg-white border-x border-[#cdd5de] px-3 sm:px-6 py-4 sm:py-6 shadow-xs">
              ...
              {groupedCategories.map(({ category, items }) => {
                ...
                return (
                  <section key={category.id} className="space-y-1">
                    {/* Classic 2010 Authority Section Header Strip (Clickable with smooth feedback) */}
                    <div
                      onClick={() => toggleCategory(category.id)}
                      className="authority-section-header px-3 py-1.5 flex items-center justify-between text-xs sm:text-sm font-bold cursor-pointer select-none hover:brightness-95 transition-all shadow-xs active:scale-[0.998]"
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 text-[11px] font-normal opacity-90">
                        <span>{dict.searchAndFilter.indicatorsCount(items.length)}</span>
                    ...
                    <div className={`accordion-wrapper ${isCollapsed ? 'collapsed' : ''}`}>
                      <div className="accordion-inner">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-1.5 pb-1">
                          {items.map((item) => (
                            <CounterCard
                              key={item.id}
                              item={item}
