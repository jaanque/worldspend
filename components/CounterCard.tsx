...
export const CounterCard: React.FC<CounterCardProps> = ({
  ...
  return (
    <div
      ...
      className={`bg-white border border-[#c8d1db] rounded-xs px-3 sm:px-4 py-2.5 sm:py-3 flex flex-row items-center justify-between gap-3 relative shadow-2xs hover:border-[#1c4b78] hover:bg-[#fafbfc] transition-all cursor-pointer group ${
        showEmbedBox || showTooltip ? 'z-40' : 'hover:z-30'
      }`}
    >
      {/* Left side: Title and Number stacked compactly */}
      <div className="flex flex-col gap-1 flex-1 min-w-0 justify-center">
        <h3 className="text-xs sm:text-[13px] font-bold text-[#14324f] leading-snug group-hover:text-[#1c4b78] group-hover:underline line-clamp-2">
          {item.title}
        </h3>
        <div className="text-lg sm:text-xl font-black text-[#112d4a] tabular-numbers select-all tracking-tight leading-none">
          {formatCurrencyValue(currentSpend, activeCurrency, activeCurrency.code === 'BTC')}
        </div>
      </div>

      {/* Right side: Action Buttons (Info & Embed) vertically centered */}
      <div
      ...
      >
        ...
        <div className="relative inline-flex items-center">
          ...
          {showTooltip && (
            <div
              onClick={(e) => e.stopPropagation()}
              className="absolute right-0 bottom-full mb-2 z-50 w-[280px] sm:w-72 max-w-[calc(100vw-2rem)] p-3 bg-white text-gray-800 text-[11px] rounded-xs shadow-xl border-2 border-[#1c4b78] cursor-default text-left origin-bottom-right"
            >
        ...
        <div className="relative inline-flex items-center">
          ...
          {showEmbedBox && (
            <div
              onClick={(e) => e.stopPropagation()}
              className="absolute right-0 bottom-full mb-2 z-50 w-[300px] sm:w-80 max-w-[calc(100vw-2rem)] p-2.5 bg-white text-gray-800 text-[11px] rounded-xs shadow-2xl border-2 border-[#1c4b78] cursor-default text-left origin-bottom-right"
            >
              <div className="flex items-center justify-between gap-1 mb-1.5 pb-1 border-b border-gray-200">
                <span className="font-bold text-[#14324f] text-xs">
